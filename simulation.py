...
try:
    ...
    from theme import apply_theme, config_banner, page_header, section_label
...
def phase_play():
    ...
    if not locked:
        branch_triggered = _check_branch_triggers_auto(scenario, dp, current_selections)
        if branch_triggered:
            for action in actions:
                if action.get("id", "") in current_selections:
                    st.session_state.sim_elapsed_minutes += compute_action_duration(action)
            if dp_id not in st.session_state.sim_completed_dps:
    ...
    if not locked:
        ...
        if feedback and current_selections:
...
def _check_branch_triggers_auto(scenario, dp, selected_actions) -> bool:
